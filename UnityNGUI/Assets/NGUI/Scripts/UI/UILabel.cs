...
public class UILabel : UIWidget
{
    ...
    [HideInInspector] [SerializeField] public string mOriginalText = string.Empty;
    ...
    public string originalText
    {
        ...
        set
        {
            ...
            if (string.IsNullOrEmpty(value))
            {
                ...
                {
                    mOriginalText = string.Empty;
                    shouldBeProcessed = true;
                    ProcessAndRequest();
                }
            }
            else if (mOriginalText != value)
            {
                mOriginalText = value;
                shouldBeProcessed = true;
                ProcessAndRequest();
    ...
    public string text
    {
        ...
        set
        {
            ...
            {
                originalText = string.Empty;      //清空字符转义符 
                value = value.Replace("\\n", "\n"); // 解决\n无法换行的问题
                mText = value;
                shouldBeProcessed = true;
                ProcessAndRequest();
    ...
    void ProcessText(bool legacyMode, bool full)
    {
        ...
        if (mPrintedSize > 0)
        {
            ...
            for (int ps = mPrintedSize; ps > 0; --ps)
            {
                ...
                // Wrap the text
                //--------------------   (添加国际化编辑器编辑方式) YangDan  -----------------------------------
                //bool fits = NGUIText.WrapText(mText, out mProcessedText, true);
                if (mOriginalText != string.Empty)
                {
                    if (moQuery != null)
                    {
                        mText = moQuery(mOriginalText);
                    }
                    else
                    {
                        //当获取不到UIStrings时，返回字符串本身
                        mText = mOriginalText;
                    }
                }
    ...
    public static void SetQueryString(OnQueryString onQueryString)
    {
        moQuery = onQueryString;
    }
